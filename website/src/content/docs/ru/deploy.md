...
Эта страница описывает прямой legacy-путь `meridian deploy`. Для новой управляемой V4-топологии используйте `meridian setup`; её `topology_intent` остаётся авторитетным для последующих изменений.
...
Эти настройки относятся к legacy PWA-страницам и сохраняются в `cluster.yml`. V4 выдаёт канонические подписки Remnawave без отдельной пользовательской PWA-страницы.
...
Значение по умолчанию (`www.microsoft.com`) подходит в большинстве случаев. Чтобы найти альтернативы рядом с сервером, просканируйте его IPv4-подсеть закреплённой версией RealiTLScanner с проверкой контрольной суммы:

```
meridian scan 198.51.100.10
```

RealiTLScanner поддерживает только IPv4; для IPv6-only сервера укажите `--sni` явно, иначе `scan` завершится кодом `3`. Выбор сохраняется только для отслеживаемого legacy-узла. В V4 команда его показывает, но intent изменяется только через `meridian setup`.
...
Выбирайте стабильное имя, которое разрешается и выполняет TLS-handshake с сервера. `meridian scan` проверяет домены из реально наблюдаемых сертификатов; `meridian preflight --sni HOST` проверяет ручной выбор, не передавая адрес сервера ASN-сервису.
...
Без установки проверяет доступность и DNS цели SNI, локальную и внешнюю доступность порта, DNS дополнительного домена, совместимость ОС, место на диске и расхождение часов. Код `0` означает готовность, `4` — обнаруженные проблемы, `3` — недоступность обязательных данных.
