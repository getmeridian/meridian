...
ПОСЛЕ УСТАНОВКИ, ЧТО-ТО СЛОМАЛОСЬ → meridian doctor IP
  ...
  Собирает: ОС сервера, Docker, логи панели и узла Remnawave, порты, фаервол, SNI, DNS.
```
...
**Решение:** `meridian teardown IP && meridian deploy IP` заново создаёт узел с чистой конфигурацией. Чтобы проверить состояние панели Remnawave, откройте интерфейс администратора по адресу `https://<IP>/<secret_path>/` и перейдите в **Nodes**: узел должен иметь статус `connected`. Команда `meridian fleet status` показывает те же сведения в CLI.
...
**Самая частая причина:** IP-адрес сервера был заблокирован. Запустите `meridian test IP`: если проверка TCP не проходит, IP, скорее всего, заблокирован.
...
| Логи Remnawave | Ошибки бэкенда панели или узла, записи "failed to start", проблемы с сертификатами |
